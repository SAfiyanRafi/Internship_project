...
export default async function BookingsPage() {
  ...
  const groups = await prisma.travelGroup.findMany({ orderBy: { id: 'desc' }, select: { id: true, groupName: true } });
  
  const bookings = await prisma.booking.findMany({
    include: { customer: true, package: true, payments: true, group: true },
    orderBy: { id: 'desc' },
    take: 500,
  });
  ...
  return (
    <div className="space-y-8">
      ...
      <BookingsClient initialBookings={bookings} customers={customers} packages={packages} groups={groups} branches={branches} currency={currency} />
    </div>
  );
}
